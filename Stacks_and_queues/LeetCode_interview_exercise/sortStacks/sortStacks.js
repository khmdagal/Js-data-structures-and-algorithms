...
============ Tried ==========
function sortStack(stack_to_sort = new Stack()){

    const additionalStack = new Stack();
    while (!stack_to_sort.isEmpty()) {
       let temp = stack_to_sort.pop();

       while(!additionalStack.isEmpty() && additionalStack.pop() > temp){
        stack_to_sort.push(additionalStack.pop())
        additionalStack.push(temp)
    ...
    return stack_to_sort.getStackList()
...
/*
     let tempVar = stack_to_sort.pop();

    if(tempStack.isEmpty()){
        tempStack.push(tempVar)
    } else if(!tempStack.isEmpty()){
        let peekedItem = tempStack.peek()
        if(peekedItem > tempVar){
           let popedItem = tempStack.pop()
            tempStack.push(tempVar)
            stack_to_sort.push(popedItem)
        }
